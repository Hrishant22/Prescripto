...
const Dashboard = () => {
  const { aToken, dashData, getDashData, cancelAppointments } =
            ...
            {dashData.latestAppointments.map((item, index) => (
              <div className="flex items-center px-6 py-3 gap-3 hover:bg-gray-100" key={index}>
                ...
                <div className="flex-1 text-sm">
                  <p className="text-shadow-gray-800 font-medium">{item.docData.name}</p>
                  <p className="text-shadow-gray-600">{slotDateFormat(item.slotDate)}</p>
                  </div>
                  {
                    item.cancelled
